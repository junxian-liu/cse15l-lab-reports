...
To log in, we would type "ssh" followed with our ieng6 account. Then when prompted to enter our password, we would then enter our password there. If everything is correct, you would get a message saying you succesffully logged into your account.
...
command, the next message we should see is whether or not the tests passed, and if they failed, the failure message. As for us, we confirmed that the tests failed. 
...
commit. After preparing those edits to be committed, I then typed `git commit -m "Updated"` to commit the edits to my Github account. That command
commited the changed with the message of "Updated".
